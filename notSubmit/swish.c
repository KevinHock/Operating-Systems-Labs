...
int executeCommand(process_list *pa){
  ...
  if(timed){
	double timetaken = (double)(end->tv_usec) - (double)(start->tv_usec);
	printf("\nTIMES: milliseconds=%fs\n", timetaken);
  }
...
int pipeTo(process_list *pl){
  //Make array of size pl->num_processes
  int** howManyFork() = malloc(pl->num_processes*);
...
int parseCommand(process *p, char *cmd){
  ...
  // HANDLE NO REDIRECTS  
  if(countIn('<', cmd) == 0 && countIn('>', cmd) == 0){
	p->args = buildArgs(cmd);
  }

  // HANDLE A SINGLE INPUT REDIRECT
  // FILE INTO COMMAND
  if(countIn('<', cmd) == 1 && countIn('>', cmd) == 0){
	    args = strtok(cmd, "<");
	    file = strtok(NULL, "<");
  ...
  // COMMAND INTO FILE
  if(append || (countIn('<', cmd) == 0 && countIn('>', cmd) == 1)){
	args = strtok(cmd, ">");
	file = strtok(NULL, ">");
	...
	if(junk != args){
	  *junk = '\0';
	  ...
	  fd = atoi(junk);
	  if(debug){
	    printf("Redirecting from FD %d\n", fd);
	    printf("CUSTOM FD A is %d\n",fd);
      }
    ...
    if(append)
      ...
      filenumber = open(file, O_CREAT | O_RDWR | O_TRUNC, S_IRUSR | S_IWUSR);
